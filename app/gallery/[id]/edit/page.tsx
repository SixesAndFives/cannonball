...
export default function GalleryItemEditorPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  // Fetch gallery item
  useEffect(() => {
    // First get all gallery items to find the album ID
    fetch('/api/albums/all/gallery')
      .then(res => res.json())
      .then((items: GalleryItem[]) => {
        ...
        setTaggedUsers(item.tagged_users || []);
      })
      .catch(console.error);
  }, [id]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!item?.album_id) throw new Error('No album ID found');
      const response = await fetch(`/api/albums/${item.album_id}/gallery/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ caption, tagged_users: taggedUsers })
      });
      ...
      console.log('[Editor] Save successful:', updatedItem);
      router.back();
    } catch (error) {
  ...
  const handleDelete = async () => {
    try {
      ...
      console.log('[Editor] Delete successful');
      router.back();
    } catch (error) {
